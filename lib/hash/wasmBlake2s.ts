...
export function buildMerkleTree(depth: number, leaves: Buffer[]): ArrayBuffer {
    ...
    hashLeaves(leaves, nRef + parentCount * digestSize, evenLeafCount);
    ...
    let tIndex = (parentCount - 1) * digestSize;
    ...
    while (parentCount > 1024) {
        ...
        tIndex = (parentCount - 1) * digestSize;
        tRef = nRef + tIndex;
        sRef = nRef + (tIndex << 1);
    }
...
// HELPER FUNCTIONS
// ================================================================================================
function hashLeaves(leaves: Buffer[], resRef: number, evenLeafCount: number) {
    for (let j = 0; j < evenLeafCount; j += 2, resRef += digestSize) {
...
function hashParents(vRef: number, resRef: number, vElementSize: number, vElementCount: number) {
